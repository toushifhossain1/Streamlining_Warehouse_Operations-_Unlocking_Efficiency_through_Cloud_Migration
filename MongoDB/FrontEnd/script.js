$(document).ready(function () {
    ...
    // Function to create methane chart
    function createMethaneChart(data) {
        const ctx = document.getElementById("myChart").getContext("2d");
        const labels = data.map(entry => entry.data.datetime);
        const methaneData = data.map(entry => entry.data.methane);
    ...
    function createTemperatureChart(data) {
        const tempChart = document.getElementById("myTempChart").getContext("2d");
        const labels = data.map(entry => entry.data.datetime);
        const temperatureData = data.map(entry => entry.data.temperature);
    ...
    function createHumidityChart(data) {
        const humidityChart = document.getElementById("myHumidityChart").getContext("2d");
        const labels = data.map(entry => entry.data.datetime);
        const humidityData = data.map(entry => entry.data.humidity);
